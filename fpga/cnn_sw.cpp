...
void preprocess(
  ...
  static float inputs[IN_NUM][IN_H_HW+10][IN_W_HW+10] = {{{0}}};
  ...
  if(LAYER>2){
    ...
    for(int ch=0; ch<IN_NUM; ch++){
      for(int h=0; h<IN_H_HW + 10; h++){
        for(int w=0; w<IN_W_HW + 10; w++){
          inputs[ch][h][w] = 0;
        }
      }
    }
    ...
    #define IN_H_HW 260
    #define IN_W_HW 260
    #define CIN_OFFSET 524288 //528464
    ...
    // Initialize the hardware input buffer
    // Cin layout: [IN_NUM / IN_NUM_T][IN_H + K - 1][IN_W + K - 1][IN_NUM_T]
    cout<<IN_H_HW<<" "<<IN_W_HW<<" "<<IN_NUM_HW<<" "<<IN_NUM_T<<" "<<IN_NUM<<" "<<CIN_OFFSET<<endl;
    for (int i1 = 0; i1 < IN_NUM_HW/IN_NUM_T; i1++){
      for (int h = 0; h < IN_H_HW; h++){
        for (int w = 0; w < IN_W_HW; w++){
          for (int i2 = 0; i2 < IN_NUM_T; i2++){//IN_NUM should be 8
    ...
    // save_progress(cin_hw, CIN_OFFSET + 540800);
    // exit(0);
  }
